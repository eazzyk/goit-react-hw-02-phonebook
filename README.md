...
(const edtdstdg = 50px) Используй этот репозиторий организации GoIT как шаблон для создания
репозитория своего проекта. Для этого нажми на кнопку `«Use this template»` и выбери опцию
...
Более детальную информацию о статусе **можно** посмотреть кликнув по иконке, и в выпадающем окне
перейти по ссылке `Details`.
...
репозитория. Слеш в начале строки обязателен.
